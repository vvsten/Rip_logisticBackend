...
func (r *Repository) GetLogisticRequests(status string, dateFrom, dateTo *time.Time) ([]ds.LogisticRequest, error) {
	...
	// lab8: добавляем поле-счётчик заполненных результатов по м-м
	if err := r.FillAsyncResultsCount(orders); err != nil {
		return nil, err
	}
...
// ==================== ЛАБ8: ASYNC RESULT (м-м) ====================

// SetAsyncResultIfEmpty записывает результат по м-м записи только если поле ещё пустое.
// Возвращает true, если удалось записать; false, если уже было заполнено.
func (r *Repository) SetAsyncResultIfEmpty(orderID, transportServiceID int, result string) (bool, error) {
	if strings.TrimSpace(result) == "" {
		return false, fmt.Errorf("result must be non-empty")
	}

	tx := r.db.Model(&ds.LogisticRequestService{}).
		Where("logistic_request_id = ? AND transport_service_id = ? AND (async_result IS NULL OR async_result = '')", orderID, transportServiceID).
		Update("async_result", result)
	if tx.Error != nil {
...
// ForceSetAsyncResult перезаписывает результат по м-м записи (для демонстрации "изменить результат" по ключу).
func (r *Repository) ForceSetAsyncResult(orderID, transportServiceID int, result string) error {
	if strings.TrimSpace(result) == "" {
		return fmt.Errorf("result must be non-empty")
	}
	tx := r.db.Model(&ds.LogisticRequestService{}).
		Where("logistic_request_id = ? AND transport_service_id = ?", orderID, transportServiceID).
		Update("async_result", result)
	return tx.Error
}

// FillAsyncResultsCount заполняет поле LogisticRequest.AsyncResultsFilledCount для списка заявок.
func (r *Repository) FillAsyncResultsCount(orders []ds.LogisticRequest) error {
	if len(orders) == 0 {
		return nil
	}
	...
	type row struct {
		LogisticRequestID int `gorm:"column:logistic_request_id"`
		Cnt               int `gorm:"column:cnt"`
	}
	var rows []row

	err := r.db.Model(&ds.LogisticRequestService{}).
		Select("logistic_request_id, COUNT(*) as cnt").
		Where("logistic_request_id IN ? AND async_result IS NOT NULL AND async_result <> ''", ids).
		Group("logistic_request_id").
		Scan(&rows).Error
	if err != nil {
		return err
	}
	...
	for _, rr := range rows {
		m[rr.LogisticRequestID] = rr.Cnt
	}

	for i := range orders {
		orders[i].AsyncResultsFilledCount = m[orders[i].ID]
	}
	return nil
}
...
func (r *Repository) RemoveTransportServiceFromGuestDraftLogisticRequest(serviceID int) error {
	...
	if err == sql.ErrNoRows {
		return fmt.Errorf("услуга не найдена в черновике заявки")
	}
	if err != nil {
		return err
	}
